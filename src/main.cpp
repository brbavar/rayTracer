...
Matrix Matrix::cross(Matrix other)
{
        ...
        entries[0].size() == 3 && other.entries[0].size() == 3)
    {
        list.reserve(3);
        for (int i = 0; i < 3; i++)
            list.push_back(entries[0][i == 2 ? 0 : i + 1] * other.entries[0][i == 0 ? 2 : i - 1] - entries[0][i == 0 ? 2 : i - 1] * other.entries[0][i == 2 ? 0 : i + 1]);
        result.entries.push_back(list);
    }
    return result;
}
...
double setBrightness(Matrix light, Matrix nearestPt, Matrix normalDir, char *pixel, Color &clr);
void render(Matrix light, int picHeight, int picWidth, std::vector<Sphere *> spheres);
...
int main()
{
    ...
    std::default_random_engine randomNumGenerator(seed);
    std::uniform_real_distribution<double> distribution(0, 1);

    // Select random RGB values
    double blue = distribution(randomNumGenerator);
    double green = distribution(randomNumGenerator);
    double red = distribution(randomNumGenerator);

    // Paint sphere the color corresponding to random RGB values
    Color colorOfSphere = Color(blue, green, red);

    // Create matrix containing x-, y-, z-coordinates of point at center of sphere
    Matrix center = Matrix(350, 210, 0);

    // Initialize pointer to sphere object; construct this object using random color and hard-coded center
    Sphere *ptr = new Sphere(center, 170, colorOfSphere);

    // Fill a vector with pointers to however many spheres you want in the image
    std::vector<Sphere *> spheres;
    spheres.reserve(1);
    spheres.push_back(ptr);

    // Place light at (400, 500, -400), i.e., x = 400, y = 500, z = -400
    Matrix light = Matrix(400, 500, -400);

    int picHeight = 500, picWidth = 700;

    // Display message indicating that rendering is in progress
    printf("Rendering...\n");

    // Create shaded image of spheres, pointers to which are in the spheres vector
