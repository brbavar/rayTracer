...
std::vector<Matrix> Shape::intersects(Ray ray) {
    ...
    if(type == "Sphere") {
        ...
        if(b * b - 4 * a * c > 0) {
            ...
            /* std::cout << "globRadius = " << globRadius << std::endl;
            std::cout << "mag of (globCenter - ptBtwn) = " << (globCenter - ptBtwn).mag() << std::endl;
            std::cout << "medDist = " << medDist << std::endl;
            std::cout << "offset = " << offset << std::endl;
            std::cout << "lilDist = " << lilDist << std::endl;
            std::cout << "ray.entries.size() = " << ray.entries.size() << std::endl;
            for(double e : ray.entries[0])
                std::cout << e << std::endl;
            std::cout << "ray.unitDir.entries.size() = " << ray.unitDir.entries.size() << std::endl;
            for(double e : ray.unitDir.entries[0])
                std::cout << e << std::endl; */
            ...
            /* for(double e : points[0].entries[0])
                std::cout << e << std::endl; */
...
void render(Light light, int picHeight, int picWidth, std::vector<Shape> shapes) {
    ...
    for(double z = - picHeight / 2; z < picHeight / 2; z++) {
        for(double x = - picWidth / 2; x < picWidth / 2; x++) {
                ...
                if(!s.intersects(cam).empty()) {
                    ...
                    for(Matrix m : s.intersects(cam)) {
                        ...
                        if(minDist == cam.distanceTo(m)) {
                            ...
                            std::cout << m.entries.size() << std::endl;
                            for(double e : m.entries[0])
                                std::cout << e << " "; */
                            nearestPt = m;
                            /* for(double e : nearestPt.entries[0])
                                std::cout << e << " "; */
            ...
            if(points.empty() || nearestPt.entries[0][0] == -1 /* || shaded */) {
            ...
            std::cout << "b = " << clr.blue << ", g = " << clr.green << ", r = " << clr.red << std::endl; */
...
int main() {
    ...
    std::uniform_real_distribution<double> y2Distro(100, 850);
    ...
    if(ans == 'c') {
        ...
        for(int i = 0; i < shapeCount; i++) {
            ...
            shapes.push_back(Sphere(center, radius, clr));  // Destructor caused this to malfunction
