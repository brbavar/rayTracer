...
double setBrightness(std::string, std::pair<Matrix,Matrix>, Matrix, Matrix, char*, Matrix&);
...
double setBrightness(std::string type, std::pair<Matrix,Matrix> light, Matrix point, Matrix normalDir, char* pixel, Matrix& clr) {
    double brightness = 1, angle;

    if(type == "Sphere") {
        // Get direction of light beam aimed at point of intersection.
        Matrix lightDir = (point - light.first).normalize();
    ...
    /* if(type == "Plane") {
        double dist = (point - light.first).mag();
        brightness /= 4 * M_PI * dist * dist;
    } */
    ...
    for(double z = -picHeight / 2; z < picHeight / 2; z++)
        for(double x = -picWidth / 2; x < picWidth / 2; x++) {
            ...
            if(minDist < INF) {
                ...
                std::string type = std::get<0>(nearestShape);
                ...
                std::pair<Matrix,Matrix> shadow = {nearestPt, light.first};
                for(auto s : shapes) {
                    if(s != nearestShape) {
                        dist = intersect(shadow, s);
                        if(dist != -1)
                            unshaded = false;
                    }
                }
...
int main() {
    ...
    if(ans == 'c') {
        ...
        if(onFloor) {            
            ...
            std::pair<Matrix,Matrix> normal = {center, Matrix(0,400,279)};
